...
static bool kinetis_fccob_cmd(target *const t, const uint8_t cmd, uint32_t addr, const uint32_t *data, int n_items)
{
	uint8_t fstat;
...
static int kinetis_flash_cmd_write(struct target_flash *const f, target_addr dest, const void *src, size_t len)
{
...
static bool backdoor_access_key_cmd(target *t, int argc, const char **argv)
{
	if (argc != 2)
		return 1;
	...
	if (fccob_command(t, FTFx_CMD_BACKDOOR_ACCESS, 0, key_vals, 2))
		return 0;
