...
#define RENESAS_IDCODE_RA6M2 0x0015
#define RENESAS_IDCODE_RA4M3 0x0031
...
const struct command_s renesas_cmd_list[] = {
	{"uid", (cmd_handler)renesas_uid, "Prints unique number"},
	{NULL, NULL, NULL},
};

struct renesas_priv_s {
...
static bool renesas_pnr_read(target *t, const uint32_t base, uint8_t *pnr)
{
	...
	if (base == RENESAS_FIXED1_PNR) {
		...
		/* see 'Part numbering scheme' note for context */
		for (size_t i = 0U; i < 13U; i++)
			pnr[i] = pnrr[3U - (i + 3U) / 4U] >> (24U - ((i + 3U) % 4U) * 8U);
		memset(pnr + 13, 0x20, 3);
	} else {
		for (size_t i = 0; i < 16; i++)
			pnr[i] = pnrr[i / 4U] >> (i % 4U) * 8U;
	}
...
static uint32_t renesas_flash_size(const uint8_t *pnr)
{
	...
	case PNR_MEMSIZE_1_5MB:
		return 1536UL * 1024UL;
	case PNR_MEMSIZE_2MB:
		return 2048UL * 1024UL;
	default:
		return 0;
	}
}

typedef enum pe_mode_e {
	PE_MODE_READ,
	PE_MODE_CF,
	PE_MODE_DF,
} pe_mode_t;

static bool renesas_rv40_pe_mode(target *t, pe_mode_t pe_mode)
{
	/* See "Transition to Code Flash P/E Mode": Section 47.9.3.3 of the RA6M4 manual R01UH0890EJ0100. */

	struct renesas_priv_s *priv_storage = (struct renesas_priv_s *)t->target_storage;
	if (!priv_storage)
		return false;
...
static int renesas_rv40_flash_erase(struct target_flash *f, target_addr addr, size_t len)
{
	target *t = f->t;
...
static int renesas_rv40_flash_write(struct target_flash *f, target_addr dest, const void *src, size_t len)
{
	target *t = f->t;
...
static void renesas_add_rv40_flash(target *t, target_addr addr, size_t length)
{
	...
	if (code_flash) {
		...
		f->buf_size = RV40_CF_REGION1_BLOCK_SIZE; /* min write size is 128B */
	} else {
		f->blocksize = RV40_DF_BLOCK_SIZE;
		f->buf_size = RV40_DF_BLOCK_SIZE; /* min write size is 4B */
	}

	target_add_flash(t, f);
}

static void renesas_add_flash(target *t, target_addr addr, size_t length)
{
	struct renesas_priv_s *priv_storage = (struct renesas_priv_s *)t->target_storage;
	if (!priv_storage)
		return;
...
bool renesas_probe(target *t)
{
	...
	default:
		/*
		 * unknown idcode, we know this AP is from renesas, so Let's try brute forcing
		 ...
		 */
		/*
		 * ra2a1 *undocummented (idcode + pnr loc wanted)
		 * ra4m1 *undocummented (idcode + pnr loc wanted)
		 * ra4m2 *undocummented (idcode + pnr loc wanted)
		 * ra4w1 *undocummented (idcode + pnr loc wanted)
		 */

		if (renesas_pnr_read(t, RENESAS_FIXED2_PNR, pnr)) {
			DEBUG_WARN("Found renesas chip (%.*s) with pnr location RENESAS_FIXED2_PNR and unsupported idcode "
					   "(0x%04" PRIx16 ") please report it\n",
				sizeof(pnr), pnr, t->idcode);
			break;
		}

		if (renesas_pnr_read(t, RENESAS_FIXED1_PNR, pnr)) {
			DEBUG_WARN("Found renesas chip (%.*s) with pnr location RENESAS_FIXED1_PNR and unsupported idcode "
					   "(0x%04" PRIx16 ") please report it\n",
				sizeof(pnr), pnr, t->idcode);
			break;
		}
		...
		flash_root_table = renesas_fmifrt_read(t);
		if (renesas_pnr_read(t, RENESAS_FMIFRT_PNR(flash_root_table), pnr)) {
			DEBUG_WARN("Found renesas chip (%.*s) with Flash Root Table and unsupported idcode (0x%04" PRIx16 ") "
					   "please report it\n",
				sizeof(pnr), pnr, t->idcode);
			break;
		}
	...
	struct renesas_priv_s *priv_storage = calloc(1, sizeof(struct renesas_priv_s));
